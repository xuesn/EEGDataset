...
for sub_dir in sub_list:
    ...
    for ses_dir in ses_list:
        ...
        for cdt_fname in cdt_list:
            ...
            event_id_epoch_rsvp = [i for i in range(1,20+1)]+[99]
            
            # load RSVP-label
            exp_info = pd.read_csv(exp_info_path, sep=',',
                                header=0)  
            img_class = exp_info['img_class'].values
            imgshow_time_and_flag_correct = exp_info['imgshow_time'].values
            # remove title
            img_class = img_class[1:]  
            imgshow_time_and_flag_correct = imgshow_time_and_flag_correct[1:]  
            # remove NaN
            ...
            # remove select
            imgshow_time_and_flag_correct = imgshow_time_and_flag_correct[not_nan_index]
            select_index = (imgshow_time_and_flag_correct == 'True') | (
                imgshow_time_and_flag_correct == 'False') | (
                imgshow_time_and_flag_correct == 'TRUE') | (
                imgshow_time_and_flag_correct == 'FALSE')
            img_class = img_class[~select_index]
            # remove special img
            special_img_classNO = 99
            special_img_index = (img_class == special_img_classNO)
            not_special_index = ~special_img_index
            img_class = img_class[not_special_index] 
            ...
            epochs = mne.Epochs(raw_data, events_manual, event_id=event_id_epoch_rsvp, tmin=epoch_st, tmax=epoch_end,
            ...
            # remove special img
            sample_time_electrode = sample_time_electrode[not_special_index, :, :]
